...
parser.add_argument('--phrase',        type=float,  default=0.5,
...
def main():
    ...
    for ep in range(iters, n_epochs):
        ...
        for epi in tqdm(range(n_episodes), desc='train_epoc:{}'.format(ep)):
            ...
            support, s_labels, query, q_labels, unlabel = loader_train.next_data(n_way, n_shot, n_query)#, augmentation=args['aug'])
        ...
        for epi in tqdm(range(n_episodes), desc='val epoc:{}'.format(ep)):
            ...
            # sample data for next batch
            support, s_labels, query, q_labels, unlabel = loader_val.next_data(n_test_way, n_test_shot, n_test_query, train=False)
            support = np.reshape(support, (support.shape[0]*support.shape[1],)+support.shape[2:])
            support = torch.from_numpy(np.transpose(support, (0,3,1,2)))
            query   = np.reshape(query, (query.shape[0]*query.shape[1],)+query.shape[2:])
            query   = torch.from_numpy(np.transpose(query, (0,3,1,2)))
        ...
        if cond1:
            ...
            print('val loss:{:.5f}, best acc:{:.5f}'.format(np.mean(loss_val), best_acc))

            # save model
            torch.save(model.state_dict(), 'checkpoints/%s/models/%s_model_best_val.t7' %(args['exp_name'],alg) )
            
            f = open('checkpoints/'+args['exp_name']+'/log.txt', 'a')
            print('{} best val {:.5f} {:.5f}'.format((ep+1)*n_episodes, best_loss, best_acc), file=f)
            f.close()
        ...
        if cond2:
            ...
            # save model
            torch.save(model.state_dict(), 'checkpoints/%s/models/%s_model_best_loss.t7' % (args['exp_name'], alg))

            f = open('checkpoints/' + args['exp_name'] + '/log.txt', 'a')
            print('{} best loss {:.5f} {:.5f}'.format((ep + 1) * n_episodes, best_loss, np.mean(acc_val)), file=f)
            f.close()
